...
import com.example.rgb4u.ver1_app.ui.activity.calendar.CalenderChangedDayActivity
import com.example.rgb4u.ver1_app.ui.activity.summary.SummaryChangedDayActivity
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.DatabaseReference
import com.google.firebase.database.FirebaseDatabase

class EmotionReselectActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        toolbar.findViewById<TextView>(R.id.toolbar_write_title).text =
            toolbarDate // 툴바 타이틀 설정 (toolbar_write_title)
        ...
        val diaryRef =
            database.child("users").child(userId).child("diaries").child(date).child("userInput")
                .child("emotionDegree")
        ...
        buttonNext.setOnClickListener {
            ...
            val diaryRef = database.child("users").child(userId).child("diaries").child(date)
                .child("userInput").child("reMeasuredEmotionDegree")
            ...
            diaryRef.setValue(emotionData).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    intent.putExtra("date", date)  // date 전달
                    intent.putExtra("Toolbar", toolbarDate)  // toolbar 전달
                    startActivity(intent)
                    finish()  // 현재 액티비티 종료
                } else {
                    // 저장 실패 시 에러 로그 출력
                    Log.e(
                        "EmotionReselectActivity",
