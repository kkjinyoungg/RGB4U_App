...
class EmotionReselectActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        squareView.setImageResource(R.drawable.ic_seekbar_star_1)
        ...
        seekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                dynamicTextView.text = when (progress) {
                    0 -> {
                        squareView.setImageResource(R.drawable.ic_seekbar_star_1) // 이미지 리소스 설정
                        "매우 심하지 않았어"
                    }
                    1 -> {
                        squareView.setImageResource(R.drawable.ic_seekbar_star_2) // 이미지 리소스 설정
                        "심하지 않았어"
                    }
                    2 -> {
                        squareView.setImageResource(R.drawable.ic_seekbar_star_3) // 이미지 리소스 설정
                        "보통이었어"
                    }
                    3 -> {
                        squareView.setImageResource(R.drawable.ic_seekbar_star_4) // 이미지 리소스 설정
                        "심했어"
                    }
                    4 -> {
                        squareView.setImageResource(R.drawable.ic_seekbar_star_5) // 이미지 리소스 설정
                        "매우 심했어"
                    }
                    else -> ""
                }
